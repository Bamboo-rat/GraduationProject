...
export default function Auth() {
  ...
  /**
   * Nếu người dùng đã đăng nhập, tự động chuyển hướng họ khỏi trang đăng nhập.
   */
  useEffect(() => {
    ...
    if (!isAuthLoading && isAuthenticated && !hasRedirectedRef.current) {
      ...
      const isAdmin = user && (
        ...
        user.roles.some(role => 
          role.includes('SUPER_ADMIN') || role.includes('STAFF') || role.includes('MODERATOR')
        )
      );

      if (isAdmin) {
        console.log('Auth.tsx: Redirecting ADMIN to', from);
        hasRedirectedRef.current = true;
        navigate(from, { replace: true });
      } else {
        console.warn('⚠️ Auth.tsx: User is authenticated but not an Admin. Logging out.');
        hasRedirectedRef.current = true; 
        logout();
      }
    }
  }, [isAuthenticated, isAuthLoading, navigate, from, user, logout]);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (rememberMe) {
        try { localStorage.setItem('remember_me', '1'); } catch {}
      } else {
        try { localStorage.removeItem('remember_me'); } catch {}
      }
    } catch (err: any) {
  ...
  // Nếu auth vẫn đang được kiểm tra, không hiển thị gì cả
  if (isAuthLoading) {
    return null; // Hoặc một spinner toàn trang
  }

  return (
    <div className="min-h-screen bg-gradient-to-br from-[#E8FFED] via-[#FFFEFA] to-[#F5EDE6] flex items-center justify-center p-4">
      {/* Decorative blobs (matching supplier style) */}
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-[#B7E4C7]">
          <div className="bg-gradient-to-r from-[#A4C3A2] to-[#2F855A] px-8 py-10 text-center relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-white/90 mt-2 font-medium">Cổng quản trị</p>
            </div>
          </div>
          ...
          <div className="px-8 py-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-sm font-semibold text-[#2D2D2D] mb-2">Tên đăng nhập</label>
                <div className="relative">
                  <input
                    ...
                    autoFocus
                  />
                  <svg className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-[#2F855A]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div>
                <label className="block text-sm font-semibold text-[#2D2D2D] mb-2">Mật khẩu</label>
                <div className="relative">
                  <input
        ...
        <div className="mt-6 text-center"><p className="text-sm text-[#8B8B8B]">© 2025 SaveFood. Cùng chống lãng phí thực phẩm</p></div>
      </div>
